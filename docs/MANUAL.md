...
cd wall-vault
make build       # bin/wall-vault (버전: v0.1.24.YYYYMMDD.HHmmss)
make install     # ~/.local/bin/wall-vault
```

> 💡 **빌드 타임스탬프 버전**: `make build`로 빌드하면 버전이 `v0.1.24.20260406.225957`처럼 날짜·시각이 포함된 형식으로 자동 생성됩니다. `go build ./...`로 직접 빌드하면 버전이 `"dev"`로만 표시됩니다.
...
#### 🔀 드래그 앤 드롭 카드 정렬 (v0.1.17)

대시보드의 에이전트 카드를 **드래그**하여 원하는 순서로 재배치할 수 있습니다.

1. 에이전트 카드를 마우스로 잡고 드래그합니다
