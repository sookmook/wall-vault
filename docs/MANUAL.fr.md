...
> 💡 **Si votre service local s'exécute sur un autre ordinateur** : Entrez l'IP de cet ordinateur dans le champ URL du service. Exemple : `http://192.168.1.20:11434` (Ollama), `http://192.168.1.20:1234` (LM Studio). Si le service n'est lié qu'à `127.0.0.1` au lieu de `0.0.0.0`, l'accès via IP externe ne fonctionnera pas — vérifiez l'adresse de liaison dans les paramètres du service.
