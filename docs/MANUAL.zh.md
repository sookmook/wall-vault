...
> 💡 **如果本地服务在另一台电脑上运行**：在服务 URL 输入框中输入那台电脑的 IP。例如：`http://192.168.0.6:11434`（Ollama），`http://192.168.0.6:1234`（LM Studio）。如果服务仅绑定到 `127.0.0.1` 而非 `0.0.0.0`，则无法通过外部 IP 访问 — 请检查服务设置中的绑定地址。
