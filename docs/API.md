...
| `key_usage` | map[string]int | Key ID → tokens used today. Vault calls `SetKeyUsage()` (absolute value sync). |
...
| `usage_update` | 프록시 heartbeat 수신 시 (20초마다) | `{"keys":[{"id","today_usage","daily_limit","cooldown_until"},...]}` | 키 사용량 바·수치 즉시 갱신, 쿨다운 카운트다운 시작. fetch 없이 SSE 데이터 직접 사용. |
...
*Last updated: 2026-03-15 — v0.1.6: avatar heartbeat sync, build timestamp versioning, agent save button fix, proxy-only service filter, avatar path support, i18n 20 new keys, key usage section redesign (SSE full key states, no extra fetch)*
