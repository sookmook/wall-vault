...
> :bulb: **Wenn ein lokaler Dienst auf einem anderen Computer läuft**: Geben Sie die IP dieses Computers in das Service-URL-Feld ein. Beispiel: `http://192.168.0.6:11434` (Ollama), `http://192.168.0.6:1234` (LM Studio). Wenn der Dienst nur an `127.0.0.1` statt `0.0.0.0` gebunden ist, funktioniert der Zugriff über externe IP nicht — prüfen Sie die Bindungsadresse in den Diensteinstellungen.
