...
*(อัปเดตล่าสุด: 2026-03-20 — v0.1.15)*
...
- **สลับบริการอัตโนมัติ (Fallback)**: หาก Google ไม่ตอบสนอง จะเปลี่ยนไปใช้ OpenRouter โดยอัตโนมัติ และหากยังไม่ได้ผล จะเปลี่ยนไปใช้ Ollama (AI ในเครื่องของคุณ) เซสชันจะไม่ขาดหาย
...
- **Base URL**: `http://ที่อยู่proxy:56244/v1`
  ...
  - เครื่องอื่น เช่น Mac Mini: `http://192.168.1.20:56244/v1`
...
เมื่อปิด VS Code การ์ด agent บนแดชบอร์ด vault จะเปลี่ยนเป็นสีเหลือง (ล่าช้า) ภายใน **2–3 นาที** และเปลี่ยนเป็นสีแดง (ออฟไลน์) ภายใน **5 นาที**
...
- **ซิงค์ checkbox อัตโนมัติ**: เมื่อเปิดหน้า หาก local service (เช่น Ollama) กำลังทำงาน checkbox จะถูกเลือกโดยอัตโนมัติ

> 💡 **หาก local service รันอยู่บนเครื่องอื่น**: ให้ป้อน IP ของเครื่องนั้นในช่อง URL ของบริการ เช่น `http://192.168.1.20:11434` (Ollama), `http://192.168.1.20:1234` (LM Studio)
