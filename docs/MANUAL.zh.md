...
> 💡 **如果本地服务在其他计算机上运行**：在服务 URL 输入框中输入该计算机的 IP。例：`http://192.168.1.20:11434`（Ollama），`http://192.168.1.20:1234`（LM Studio）。如果服务仅绑定到 `127.0.0.1` 而非 `0.0.0.0`，则无法通过外部 IP 访问——请检查服务的绑定地址设置。
