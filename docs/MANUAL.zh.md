...
> 💡 **如果本地服务在另一台电脑上运行**：在服务 URL 输入栏中输入该电脑的 IP。例：`http://192.168.1.20:11434`（Ollama）、`http://192.168.1.20:1234`（LM Studio）。如果服务仅绑定到 `127.0.0.1` 而非 `0.0.0.0`，则无法通过外部 IP 访问，请检查服务的绑定地址设置。
