...
> 💡 **If your local service is running on another computer**: Enter that computer's IP in the service URL field. Example: `http://192.168.1.20:11434` (Ollama), `http://192.168.1.20:1234` (LM Studio). If the service is bound only to `127.0.0.1` instead of `0.0.0.0`, external IP access won't work — check the binding address in the service settings.
