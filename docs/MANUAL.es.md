...
| `WV_PROXY_TLS_REQUIRED` | `proxy.tls.required` (rechaza arrancar con TLS desactivado — bloquea el fallback en texto plano) |
| `WV_PROXY_ALLOW_CIDRS` | `proxy.allow_cidrs` (lista separada por comas, p. ej. `192.168.0.0/16,10.0.0.0/8`; el loopback siempre pasa) |
...
| `WV_OLLAMA_URL` | Override de URL de Ollama por host (instancia única) |
| `WV_OLLAMA_URLS` | URLs de Ollama separadas por comas (dispatch multi-instancia) |
| `WV_OLLAMA_KEEP_ALIVE` | `proxy.ollama_keep_alive` |
| `WV_OLLAMA_NUM_CTX` | `proxy.ollama_num_ctx` |
| `WV_LMSTUDIO_URL`, `WV_VLLM_URL`, `WV_LLAMACPP_URL` | Override de URL por backend (instancia única) |
...
| `WV_INJECT_MODEL_IDENTITY` | `proxy.inject_model_identity` (guardia de identidad por mensaje del sistema, desactivado por defecto) |
| `WV_PROMPT_TOKEN_CAP` | Cap de auto-truncado por host para prompts OAI-compat locales (entero positivo = activar, 0 = off) |
| `WV_DISPATCH_TRACE` | Establecer `1` para registrar el servicio/modelo resuelto y razón de cada dispatch (off por defecto) |
