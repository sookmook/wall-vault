...
> :bulb: **如果本地服务在另一台电脑上运行**：在服务 URL 输入框中输入该电脑的 IP。例如：`http://192.168.1.20:11434`（Ollama）、`http://192.168.1.20:1234`（LM Studio）。如果服务绑定在 `127.0.0.1` 而非 `0.0.0.0`，则无法通过外部 IP 访问——请在服务设置中检查绑定地址。
