...
> 💡 **Wenn der lokale Dienst auf einem anderen Computer läuft**: Geben Sie die IP dieses Computers im URL-Feld des Dienstes ein. Beispiel: `http://192.168.1.20:11434` (Ollama), `http://192.168.1.20:1234` (LM Studio). Wenn der Dienst nur an `127.0.0.1` statt `0.0.0.0` gebunden ist, funktioniert der Zugriff über externe IP nicht — überprüfen Sie die Bindungsadress-Einstellung des Dienstes.
