...
mode: distributed
proxy:
  vault_url: http://192.168.0.6:56243
...
```bash
# 금고에서 봇A 모델 변경
curl -X PUT http://192.168.0.6:56243/admin/clients/botA \
  -H "Authorization: Bearer 관리자토큰" \
  -H "Content-Type: application/json" \
...
| `WV_VAULT_URL` | 금고 서버 URL (distributed) | `http://192.168.0.6:56243` |
...
| `OLLAMA_URL` | Ollama 서버 URL | `http://192.168.0.6:11434` |
| `VAULT_CLIENT_ID` | 클라이언트 ID (레거시 호환) | `motoko` |
...
export OLLAMA_URL=http://192.168.0.6:11434
wall-vault start
```
