...
| 필드 | 타입 | 설명 |
|------|------|------|
| `today_usage` | int | 오늘 성공한 요청 토큰 수 (429/402/582 오류는 포함되지 않음) |
| `today_attempts` | int | 오늘 총 API 호출 횟수 (성공 + rate-limited 포함) |
| `available` | bool | 쿨다운·한도 없이 사용 가능 여부 |
| `usage_pct` | int | 일일 한도 대비 사용량 % (`daily_limit=0`이면 0) |
...
| `usage_update` | 프록시 heartbeat 수신 시 (20초마다) | `{"keys":[{"id","service","today_usage","today_attempts","daily_limit","cooldown_until"},...]}` | 키 사용량 바·수치 즉시 갱신, 쿨다운 카운트다운 시작. fetch 없이 SSE 데이터 직접 사용. 바는 share-of-total 스케일링 (무제한 키). |
...
| `custom/` | 재귀 재파싱 (`custom/` 제거 후 재라우팅) | `custom/google/gemini-2.5-flash` → Google |
...
| `today_usage` | int | 오늘 성공한 요청 토큰 수 (429/402/582 오류 미포함) |
| `today_attempts` | int | 오늘 총 API 호출 횟수 (성공 + rate-limited; 자정 초기화) |
...
| 402 (Payment Required) | 24시간 |
| 400 / 401 / 403 | 24시간 |
| 582 (Gateway Overload) | 5분 |
| 네트워크 오류 | 10분 |

> **429·402·582**: 쿨다운 설정 + `today_attempts` 증가. `today_usage`는 변경 없음 (성공 토큰만 집계).
> **Ollama (로컬 서비스)**: `callOllama` 는 `Timeout: 0` (무제한) 전용 HTTP 클라이언트를 사용합니다. 대형 모델 추론은 수십 초~수 분이 소요될 수 있으므로 기본 60초 타임아웃을 적용하지 않습니다.
...
*Last updated: 2026-03-15 — v0.1.6+: today_attempts tracking (today_usage/today_attempts separation), HTTP 582 cooldown (5min), share-of-total bar scaling, custom/ routing fix, Ollama Timeout:0, key_att i18n, avatar heartbeat sync, build timestamp versioning, proxy-only service filter*
