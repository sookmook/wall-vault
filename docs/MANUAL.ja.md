...
- **サービスの自動フォールバック**: Googleが応答しなければOpenRouterへ、それも駄目なら自分のPCにインストールしたOllama（ローカルAI）へ自動的に切り替わります。セッションが途切れることはありません。元のサービスが復旧すると、次のリクエストから自動的に復帰します（v0.1.18+）。
...
- **Base URL**: `http://プロキシアドレス:56244/v1`
  ...
  - Miniサーバーなど別のマシンなら `http://192.168.0.6:56244/v1`
...
VS Codeを閉じると、ボールトダッシュボードでは約 **90秒** 後にエージェントカードが黄色（遅延）に、**3分** 後に赤色（オフライン）に変わります。（v0.1.18以降、15秒間隔のステータスチェックによりオフライン検知が高速化しました。）
...
- **ローカルサービスのステータス表示**: ページを開いたとき、ローカルサービス（Ollamaなど）が起動中であれば ● 点が緑色に変わりますが、チェックボックスの状態は変更されません。

> 💡 **ローカルサービスが別のコンピューターで動いている場合**: サービスURLの入力欄にそのコンピューターのIPを入力してください。例：`http://192.168.0.6:11434`（Ollama）、`http://192.168.0.6:1234`（LM Studio）
...
- **LM Studio / vLLM対応**: これらのローカルサービスがOllamaへフォールバックせず、正しくディスパッチされるようになりました。
- **ダッシュボード修正**: フォールバック先ではなく、設定済みサービスが常に表示されるようになりました。
- **ローカルサービスのチェックボックス保持**: ページ読み込み時にローカルサービスのチェックボックスが自動的に無効化されなくなりました。
- **ツールフィルタ環境変数**: `WV_TOOL_FILTER=passthrough` 環境変数に対応しました。

### v0.1.20 (2026-03-28)
- **包括的なセキュリティ強化**: XSS対策（41箇所）、定時間トークン比較、CORS制限、リクエストサイズ制限など。

### v0.1.19 (2026-03-27)
- **Claude Codeのオンライン検出**: プロキシをバイパスしている場合でも、ダッシュボードでClaude Codeがオンラインとして表示されるようになりました。

### v0.1.18 (2026-03-26)
- **フォールバック復帰修正**: 元のサービスが利用可能になると自動復帰します。
- **オフライン検出の改善**: 15秒間隔のステータスポーリング。

### v0.1.17 (2026-03-25)
- **ドラッグ＆ドロップによるカード並べ替え**。
- **未接続エージェント向けのインライン適用ボタン**。
- **cokacdir エージェントタイプ追加**。

### v0.1.16 (2026-03-25)
- **ClineとClaude Codeの双方向モデル同期**。
