...
- **服务自动切换（回退）**：如果 Google 没有响应，自动切换到 OpenRouter；如果还是不行，自动切换到本机安装的 Ollama（本地 AI）。会话不会断开。当原始服务恢复后，从下一个请求开始自动切换回来（v0.1.18+）。
...
- **本地服务状态指示**：打开页面时，如果本地服务（Ollama 等）正在运行，● 圆点变为绿色，但不会更改复选框状态。

> 💡 **如果本地服务运行在其他计算机上**：在服务 URL 输入框中输入那台计算机的 IP。例如：`http://192.168.1.20:11434`（Ollama）、`http://192.168.1.20:1234`（LM Studio）
...
- **LM Studio / vLLM 支持**：这些本地服务现在可以正确调度，不再回退到 Ollama。
- **仪表盘修复**：始终显示已配置的服务，而非回退服务。
- **本地服务复选框保留**：仪表盘在页面加载时不再自动取消勾选本地服务。
- **工具过滤环境变量**：支持 `WV_TOOL_FILTER=passthrough` 环境变量。

### v0.1.20 (2026-03-28)
- **全面安全加固**：XSS 防护（41 处）、恒定时间令牌比较、CORS 限制、请求大小限制等。

### v0.1.19 (2026-03-27)
- **Claude Code 在线检测**：即使绕过代理，Claude Code 也能在仪表盘上显示为在线状态。
