...
curl -X POST http://localhost:56243/admin/clients \
  -H "$ADMIN" -H "Content-Type: application/json" \
  -d '{"id":"dev-agent","name":"개발 에이전트","default_service":"google","default_model":"gemini-2.5-flash","agent_type":"claude-code","work_dir":"/home/user/project","description":"개발 보조","ip_whitelist":["192.168.0.1"],"enabled":true}'
...
curl -X POST http://localhost:56243/api/heartbeat \
  -H "Authorization: Bearer your-bot-a-token" \
  -H "Content-Type: application/json" \
  -d '{"client_id":"bot-a","version":"v0.1.0","service":"google","model":"gemini-2.5-flash","sse_connected":true}'
```
