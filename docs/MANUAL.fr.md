...
- **Base URL** : `http://adresse-proxy:56244/v1`
  ...
  - Machine différente (ex : Mac Mini) : `http://192.168.0.6:56244/v1`
...
> :bulb: **Si un service local s'exécute sur un autre ordinateur** : Entrez l'IP de cet ordinateur dans le champ URL du service. Exemple : `http://192.168.0.6:11434` (Ollama), `http://192.168.0.6:1234` (LM Studio). Si le service est lié à `127.0.0.1` plutôt qu'à `0.0.0.0`, l'accès par IP externe ne fonctionnera pas — vérifiez l'adresse de liaison dans les paramètres du service.
