...
> :bulb: **If a local service is running on another computer**: Enter that computer's IP in the service URL field. Example: `http://192.168.0.6:11434` (Ollama), `http://192.168.0.6:1234` (LM Studio). If the service is bound to `127.0.0.1` rather than `0.0.0.0`, external IP access won't work — check the binding address in the service settings.
