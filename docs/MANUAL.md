...
- **Base URL**: `http://프록시주소:56244/v1`
  ...
  - 미니 서버 등 다른 머신이면 `http://192.168.0.6:56244/v1`
...
> 💡 **로컬 서비스가 다른 컴퓨터에서 실행 중이라면**: 서비스 URL 입력란에 그 컴퓨터의 IP를 입력하세요. 예: `http://192.168.0.6:11434` (Ollama), `http://192.168.0.6:1234` (LM Studio). 서비스가 `0.0.0.0`이 아닌 `127.0.0.1`에만 바인딩되어 있으면 외부 IP로 접속이 안 되므로, 서비스 설정에서 바인딩 주소를 확인하세요.
